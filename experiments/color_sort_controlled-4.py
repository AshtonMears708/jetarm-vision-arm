...
Structure:
    - classify_fruit()    — runs YOLOv11n to identify fruit shown to camera
    - decide_pick_order() — filters detected blocks by the fruit-driven goal color
    - PLACE_POSITIONS     — define where each color gets dropped
    - TARGET_COLORS       — which colors to look for
    - FRUIT_TO_COLOR      — maps fruit class name to pick color goal
    - FRUIT_SCAN_HOME     — servo positions for horizontal fruit-scanning pose
...
# Fruit scan position — arm looks horizontally to classify fruit shown to camera
# Update these servo values after physically positioning the arm
FRUIT_SCAN_HOME = ((1, 500), (2, 520), (3, 210), (4, 50), (5, 550))


# Mapping from YOLO fruit class name to the pick color goal
FRUIT_TO_COLOR = {
    'fork':  'red',
    'knife': 'green',
    'spoon': 'blue',
...
def decide_pick_order(detected_blocks, goal_color):
    """
    Filters detected blocks to only those matching the fruit-driven goal color.
    Returns the highest-confidence match (largest contour area), or None if
    no goal is set or no matching block is visible.
...
class ControlledColorSort(Node):
    def __init__(self, name):
        ...
        # Mode state — the system alternates between scanning for fruit and picking objects
        # 'fruit'  — arm at FRUIT_SCAN_HOME, YOLO classifying fruit shown to camera
        # 'pick'   — arm looking down at table, OpenCV finding matching colored object
        self.mode         = 'fruit'
        self.goal_color   = None
        self.fruit_scores = []
        self.fruit_timer  = time.time()

        # Load the YOLO model for fruit classification
        self.fruit_model = YOLO('/home/ubuntu/third_party/yolo/yolo11n.pt')
        ...
        # Move to fruit scan position — arm looks horizontally at startup
        set_servo_position(self.joints_pub, 1.0, FRUIT_SCAN_HOME)
        time.sleep(1.5)
    ...
    def pick_and_place(self, block):
        ...
        self.get_logger().info('Done. Returning to fruit scan mode.')
        self.target       = None
        self.goal_color   = None
        self.fruit_scores = []
        self.fruit_timer  = time.time()
    ...
    def classify_fruit(self, rgb_image):
        """
        Runs YOLO on the current frame to detect an object shown to the camera.
        Returns a tuple of (goal_color, confidence) if a relevant object is
        detected, or (None, 0.0) if nothing relevant is seen.
        """
        bgr_image = cv2.cvtColor(rgb_image, cv2.COLOR_RGB2BGR)
        results = self.fruit_model(bgr_image, verbose=False, conf=0.5)
        ...
        for r in results:
            for box in r.boxes:
                label = self.fruit_model.names[int(box.cls)]
                conf  = float(box.conf)
                if label in FRUIT_TO_COLOR and conf > best_conf:
                    self.get_logger().info(f'YOLO saw: {label} ({conf:.2f})')
                    best_conf  = conf
                    best_color = FRUIT_TO_COLOR[label]
    ...
    def main_loop(self):
        while True:
            ...
            if self.running:
                ...
                if self.mode == 'fruit':
                    # Fruit scanning mode — run YOLO on clean raw frame
                    detected_color, detected_conf = self.classify_fruit(raw_image)
                    now = time.time()

                    if detected_color is not None:
                        if detected_color == self.goal_color:
                            self.fruit_scores.append(detected_conf)
                        else:
                            self.goal_color   = detected_color
                            self.fruit_scores = [detected_conf]
                            self.fruit_timer  = now

                    if now - self.fruit_timer >= 0.5:
                        if self.fruit_scores and np.mean(self.fruit_scores) >= 0.70:
                            self.get_logger().info(
                                f'Object locked: {self.goal_color} '
                                f'(mean conf: {np.mean(self.fruit_scores):.2f})'
                            )
                            self.fruit_scores = []
                            self.fruit_timer  = now
                            self.mode         = 'pick'
            ...
            if result_image is not None:
                ...
                if self.mode == 'fruit' and self.fruit_scores:
                    cv2.putText(display,
                                f'Conf mean: {np.mean(self.fruit_scores):.2f} ({len(self.fruit_scores)} samples)',
                ...
                if key == ord('a'):
                    ...
                    set_servo_position(self.joints_pub, 1.0, FRUIT_SCAN_HOME)
                    time.sleep(1.5)
                    self.get_logger().info('Started. Show a fruit to the camera.')
                elif key == ord('s'):
                    self.running     = False
                    self.mode        = 'fruit'
                    self.goal_color  = None

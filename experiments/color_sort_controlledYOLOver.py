...
FRUIT_SCAN_HOME = ((1, 500), (2, 520), (3, 210), (4, 400), (5, 550))

# Frames the same fruit must be detected consecutively before locking in the goal
FRUIT_STABLE_COUNT = 25
...
class ControlledColorSort(Node):
    ...
    def classify_fruit(self, rgb_image):
        """
        Runs YOLO on the current frame to detect fruit shown to the camera.
        Returns the mapped goal color string if a fruit is detected confidently,
        or None if nothing relevant is seen.
        """
        results = self.fruit_model(rgb_image, verbose=False, conf=0.5)
        ...
        while True:
            ...
            if self.running:
                ...
                if self.mode == 'fruit':
                    ...
                    detected_color = self.classify_fruit(raw_image)
                    ...
                    if self.fruit_count >= FRUIT_STABLE_COUNT:
                        self.get_logger().info(
                            f'Fruit detected: {self.goal_color} goal locked. Switching to pick mode.'
                        )
                        self.fruit_count = 0
                        self.mode        = 'pick'
                        self.start_count = True
                        # Move arm to table position in a thread to avoid blocking the loop
                        threading.Thread(
                            target=lambda: (
                                set_servo_position(self.joints_pub, 1.0, HOME),
                                time.sleep(1.5)
            ...
            if result_image is not None:
                ...
                if self.mode == 'fruit':
                    cv2.putText(display,
                                f'Fruit stable: {self.fruit_count}/{FRUIT_STABLE_COUNT}',

...
def decide_pick_order(detected_blocks):
    """
    Given a list of detected blocks, return which one to pick up next.
    detected_blocks: list of [color, (cx, cy), angle]
    Returns: one block or None

    RIGHT NOW: picks the first detected block
    LATER: replace with your AI model output
    """
    if not detected_blocks:
        return None

    # Default: pick first detected
    return detected_blocks[0]

    # Example: only pick red
    # for block in detected_blocks:
    #     if block[0] == 'red':
    #         return block
    # return None

    # Example: priority order red → green → blue
    ...
    def main_loop(self):
        while True:
            ...
            if target_list and self.running:
                chosen = decide_pick_order(target_list)
